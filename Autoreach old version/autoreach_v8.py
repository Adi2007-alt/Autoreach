# -*- coding: utf-8 -*-
"""
AutoReach v8.0 — WhatsApp Outreach Automation
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

KEY CHANGES FROM v7:
  ✅ ENTER key sent via keyboard (pyautogui.press) — no mouse click needed for send
  ✅ Browser tab closes IMMEDIATELY after message sent (or on error)
  ✅ On any error: number stored in failed log, shown to user at end — never skips silently
  ✅ "Reset Settings" button in Settings tab — resets to factory defaults
  ✅ All settings persist across app restarts automatically
  ✅ Session NEVER disturbs user: app window stays on top, browser hidden in bg (optional)
  ✅ Full failed-numbers report shown to user at end of every session
  ✅ Smarter WhatsApp Web page-ready detection (polls title bar)
  ✅ Graceful stop: current number finishes or fails cleanly, then stops
  ✅ No hanging threads — every path ends with tab closed

Install:
    pip install pyautogui pyperclip pygetwindow Pillow

Run:
    python autoreach_v8.py
"""
...
def vary(text):
    """Insert a zero-width char into message so each is unique (anti-spam)."""
    words = text.split(" ")
    if len(words) < 4:
        return text
...
def to_e164(raw: str):
    """
    Normalise phone number → 12-digit string (91xxxxxxxxxx).
    Handles: 10-digit, +91xxx, 091xxx, 91xxx, with dashes/spaces/dots.
    """
...
DEFAULTS = {
    ...
    "hide_browser": False,
    "single_msg":   True,
    "stop_on_fail": False,   # NEW: stop session on first failure
}
...
class BrowserMgr:
    """
    Handles showing/hiding the browser window.
    - activate(): bring browser to front so pyautogui can interact with it
    - hide():     push browser to background (minimize) so user isn't disturbed
    """
    ...
    def activate(self):
        """Bring browser to front — needed before any pyautogui interaction."""
        for w in self._wins():
            try:
                w.restore()
                time.sleep(0.08)
    ...
    def hide(self):
        """Minimize browser so user doesn't see it."""
        for w in self._wins():
            try:
                w.minimize()
            except Exception:
    ...
    def close_tab(self):
        ...
        try:
            ...
            pass
        # Minimize again after closing tab
        time.sleep(0.2)
        self.hide()
...
class AutoReach:
    ...
    def __init__(self, root: tk.Tk):
        ...
        self.total          = 0
        self.sent_count     = 0
        self.failed_list    = []   # list of (number, reason) tuples
    ...
    def _build_send(self, T):
        ...
        tk.Checkbutton(top,
                       text="📋 One message (keep formatting / newlines)",
    ...
    def _build_settings(self, T):
        ...
        def spin(label, var, lo, hi, tip=""):
            tip_txt = f"  ← {tip}" if tip else ""
            tk.Label(fr, text=label + tip_txt,
                     bg=BG, fg=TEXT, anchor="w",
                     font=("Segoe UI", 9), width=58).grid(
        ...
        spin("Min delay between messages (seconds):", self.v_dmin, 5, 300, "increase = safer, slower")
        spin("Max delay between messages (seconds):", self.v_dmax, 10, 600)
        spin("Wait for WhatsApp Web page to load (seconds):", self.v_load, 3, 120, "10s recommended")
        spin("Post-send pause before closing tab (seconds):", self.v_postsend, 1, 30, "3s default")
        spin("Per-message hard timeout (seconds):", self.v_timeout, 15, 180, "forces skip if exceeded")
        ...
        # ── Behaviour ─────────────────────────────
        section("⚙  Behaviour")
        check("Skip numbers already sent (recommended)", self.v_skip_sent, GREEN)
        check("Skip blacklisted numbers (recommended)", self.v_skip_bl, GREEN)
        check("Vary each message slightly — anti-spam invisible character", self.v_vary, BLUE)
        ...
        if HAS_GW:
            check(
                ...
                "      (browser runs silently — you won't see Chrome/Edge pop up)",
                self.v_hide_browser, BLUE)
            tk.Label(fr,
                text="  The AutoReach window stays fully visible at all times.\n"
                     "  Only Chrome/Edge is minimized in the background.",
                bg=BG, fg=MUTED, font=("Segoe UI", 8),
                justify="left").grid(
                    row=row[0], column=0, columnspan=3,
    ...
    def _build_calibrate(self, T):
        ...
        tk.Label(info,
            text=(
                ...
                "Redo if you move or resize your browser window."
            ),
    ...
    def _build_about(self, T):
        ...
        tk.Label(T, text="WhatsApp Web Outreach Automation  ·  Made for efficiency",
                 bg=BG, fg=MUTED, font=("Segoe UI", 10)).pack()
        tk.Frame(T, bg=BORDER, height=1).pack(fill="x", padx=60, pady=18)
        tk.Label(T, text=(
            "What's new in v8.0\n\n"
            "  ✅  ENTER key sent via keyboard — no mouse movement for send\n"
            "  ✅  Browser tab closes immediately after send or on error\n"
            "  ✅  Failed numbers stored with reason — full report at session end\n"
            "  ✅  'Reset Settings' button — reset to factory defaults anytime\n"
            "  ✅  All settings auto-saved & restored on next launch\n"
            "  ✅  'Stop on first failure' mode (strict) — optional\n"
            "  ✅  'View Failed Numbers' button — see failures without ending session\n"
            "  ✅  Header status bar shows live state\n"
            "  ✅  Browser hider improved — activates only when needed for input\n"
            "  ✅  Smarter WA page-ready detection via clipboard probe\n"
            "  ✅  Exponential back-off on retries\n"
            "  ✅  All-time sent + blacklist count on Send tab\n"
        ),
    ...
    def _autosave(self, *_):
        try:
            ...
            self.lbl_save_status.config(
                text=f"✅ Settings saved  ({datetime.now().strftime('%H:%M:%S')})",
                fg=GREEN)
    ...
    def _reset_settings(self):
        ...
        self.lbl_save_status.config(
            text="🔁 Settings reset to defaults.", fg=AMBER)
    ...
    def _calib_refresh(self):
        ...
        src = "✅ Calibrated" if self.calib.is_set() else "⚠ Default (estimated — calibrate for accuracy)"
    ...
    def _mode_hint(self):
        if self.v_single.get():
            ...
            self.lbl_mode.config(
                text="▸ Each paragraph sent separately (split on blank line)")
        self._autosave()
    ...
    def _status(self, txt: str, color=None):
        self._ui_q.put(("status", txt, color))
        ...
        try:
            while True:
                ...
                if cmd == "log":
                    ...
                    _, txt = item[0], item[1]
                    color  = item[2] if len(item) > 2 and item[2] else AMBER
                    self.lbl_status.config(text=txt, fg=color)
                    self.lbl_hdr_status.config(text=txt.replace("●", "●"), fg=color)
                elif cmd == "stats":
    ...
    def _click_msg_box(self, restore_first=True):
        """
        Click the WhatsApp Web message input box.
        If hide_browser is ON, we restore the browser window just long enough
        to click, then minimize it again.
        """
    ...
    def _wait_page_ready(self, deadline: float) -> bool:
        """
        Poll every 0.5s until the WA Web page responds to clipboard ops.
        Returns True if ready, False if timeout or stopped.
        """
        ...
        while waited < load_max:
            ...
            # Activate browser briefly to probe
            if self.v_hide_browser.get() and HAS_GW:
                self.browser.activate()
                time.sleep(0.15)
            ...
            # Probe: select-all + copy; if clipboard changes, page is alive
            try:
            ...
            if clip is not None:
                # Page responded — click once more cleanly to ensure focus
                self._click_msg_box(restore_first=True)
                return True

            remaining = int(load_max - waited)
            self._status(f"● Waiting for WhatsApp Web… ({remaining}s)",
                         color=AMBER)
            time.sleep(0.5)
            waited += 0.5

        # Fallback: try one last click
        self._click_msg_box(restore_first=True)
        return True
    ...
    def _paste_text(self, text: str):
        """Paste text via clipboard (Ctrl+V). Browser must be active."""
        if self.v_hide_browser.get() and HAS_GW:
            self.browser.activate()
            time.sleep(0.12)
        try:
    ...
    def _safe_paste(self):
        """Return clipboard content or None on error."""
        try:
    ...
    def _press_enter(self):
        ...
        Uses keyboard (pyautogui.press) — no mouse click required.
    ...
    # ══════════════════════════════════════════
    #  CORE: WAIT FOR BOX TO CLEAR
    # ══════════════════════════════════════════
    def _wait_box_clear(self, max_sec: float = 8.0) -> bool:
        """
        After pressing Enter, the message box clears if send succeeded.
        Polls every 0.25s. Returns True = cleared (sent), False = timeout.
        """
    ...
    def _send_single(self, number: str, msg: str) -> tuple:
        ...
        Always closes the tab before returning.
        """
        timeout  = self.v_timeout.get()
        ...
        try:
            ...
            # Bring browser up for initial load
            self.browser.activate()
            time.sleep(0.4)

            # Immediately hide if user wants it
            if self.v_hide_browser.get() and HAS_GW:
                time.sleep(0.3)
                self.browser.hide()
            ...
            # Verify paste landed — retry once if empty
            if self.v_hide_browser.get() and HAS_GW:
            ...
            if (self._safe_paste() or "").strip() == "":
                # Box is empty — paste failed, try again
                self._click_msg_box(restore_first=True)
                self._paste_text(msg)
                time.sleep(0.3)
            ...
            confirmed = self._wait_box_clear(
                min(8.0, deadline - time.time()))
            ...
            for i in range(post, 0, -1):
                if not self.running:
                    break
                self._status(f"● Sent ✅  —  closing tab in {i}s…", color=GREEN)
                time.sleep(1.0)

            return True, ""

        except pyautogui.FailSafeException:
            raise   # re-raise to be caught in send_message
    ...
    def do_import(self):
        ...
        for ln in lines:
            ln = ln.strip().split(",")[0].strip()  # CSV first column
            n  = to_e164(ln)
    ...
    def do_reset(self):
        ...
        self.skipped_count = 0
        self._update_stats()
        self._status("● Idle — import a new number list", color=AMBER)
    ...
    def do_start(self):
        ...
        if self.current_idx >= self.total:
            messagebox.showinfo("All Done",
                "All numbers processed.\nUse 'Reset List' then import again."); return
    ...
    def do_pause(self):
        ...
        if self.paused:
            ...
            self._status(f"● Paused at {self.current_idx}/{self.total}", color=AMBER)
    ...
    def do_stop(self):
        ...
        self._status(f"● Stopped at {self.current_idx}/{self.total}", color=RED)
        self._log("warn",
    ...
    def do_test(self):
        val = simpledialog.askstring("Test Send",
            "Enter phone number to test (10 or 12 digits):")
        if not val: return
        n = to_e164(val.strip())
        if not n:
            messagebox.showwarning("Invalid", "Cannot parse number. Use 10 or 12 digits."); return
        raw = self.msg_box.get("1.0", tk.END).strip()
        if not raw:
            messagebox.showwarning("Empty Message", "Write a message first."); return

        self._log("info", f"🔬 Test → {n}")
        def _t():
            ok, reason = self.send_message(n, raw)
            result = "✅ Test sent!" if ok else f"❌ Test FAILED\nReason: {reason}"
            self._log("sent" if ok else "failed", result.split("\n")[0])
    ...
    def do_export(self):
        if not self.failed_list:
            messagebox.showinfo("No Failures", "No failed numbers."); return
    ...
    def do_blacklist(self):
        val = simpledialog.askstring("Blacklist",
            ...
            "(Leave blank to blacklist ALL failed numbers.)")
        ...
        if val.strip():
            ...
            if n:
                ...
                messagebox.showinfo("Done",
                    f"✅ {n} blacklisted.\n"
                    f"Total: {self.blacklist.count()}")
            else:
                messagebox.showwarni